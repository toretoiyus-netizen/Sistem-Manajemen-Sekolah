...
import { BankSoal as BankSoalType, UserAccount, MediaHotspot } from '../types';
...
export const BankSoal: React.FC<BankSoalProps> = ({ currentUser }) => {
  ...
  const filteredSoal = db.bankSoal.filter((s) => {
    ...
    return matchSearch && matchMapel && matchTingkat && matchJenis && matchMedia;
  });
  ...
  const handleGenerateQuestionsAI = async () => {
    ...
    try {
      ...
      if (data.success && Array.isArray(data.questions)) {
        ...
        alert(`Berhasil membuat ${newQuestions.length} butir soal baru dengan bantuan AI Gemini!`);
      } else {
        alert('Gagal menghasilkan butir soal otomatis. Silakan coba kembali.');
      }
    } catch (e) {
      alert('Terjadi kendala saat memanggil model AI.');
    } finally {
      setAiLoading(false);
    }
  };
